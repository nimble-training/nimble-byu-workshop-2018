...
We first need to build the model as usual. We're going to use a different 
...
The parameters and data in a model are represented as nodes in a graph. Here we define some terms:
 ...
 - node: an element in the model graph representing data, parameter, or a deterministic quantity that is a function of other quantities in the model, e.g., `p[1, 1]` or `p[1, 2]` in the litters model
 - variable: a collection of one or more nodes with the same name, e.g., `r` or `a` in the litters model
