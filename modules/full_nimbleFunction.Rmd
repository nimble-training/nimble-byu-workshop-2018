...
Setup code is written as a R function, using R code, usually including NIMBLE's special functions for [querying the model structure](modules/model_structure_slides.html).

Run code is written using the NIMBLE *domain-specific language* (DSL). While this is formally a language distinct from R, you can just think of it as a subset of R, enhanced with some functions for [operating on the model](modules/operating_model_slides.html).
...
rMargSampler <- margSampler(littersModel, samplesMarginal, 'p')
