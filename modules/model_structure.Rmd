...
The parameters and data in a model are represented as nodes in a graph. Here we define some terms:
 ...
 - node: an element in the model representing data, parameter, or a deterministic quantity that is a function of other quantities in the model, e.g., `delta[2]` in the AFT model
 - variable: a collection of one or more nodes with the same name, e.g., `delta` or `xi` in the AFT model

Note, if we have a multivariate distribution, then the node includes all of the elements, e.g.,

```
mu[1:p] ~ dmnorm(mu0[1:p], cov = Sigma[1:p, 1:p])
```

has `mu[1:p]` as a node and `mu` as a variable. 
